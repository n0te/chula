    ...
    <div class="modal-dialog">
        <div class="modal-content warning">
            <div class="modal-header">
                ...
                <h4 class="modal-title">ยืนยันการยกเลิก</h4>
            </div>
...
<script>
                            ...
                            columns: [//`bookingid``bookingdate``bookingstarttime``bookingendtime``equipmentname``equipmentpicturename`
                                ...
                                {"bVisible": true, "bSearchable": false, "bSortable": false,
                                    ...
                                    "mRender": function (data, type, full) {
                                        if (String(full.biobookingstatus) === '0') {
                                            ...
                                            return '<span class="label label-sm label-danger">ไม่เข้าพบตามกำหนด</span>';
                                        } else if (String(full.biobookingstatus) === '10') {
                                            return '<span class="label label-sm label-warning">รอการอนุมัติ</span>';
                                        } else if (String(full.biobookingstatus) === '-10') {
                                ...
                                {"bVisible": true, "bSearchable": false, "bSortable": false,
                                    ...
                                    "mRender": function (data, type, full) {
                                        ...
                                        if (String(full.biobookingstatus) === '0') {
                                            return "<a class='btn btn-info btn-sm' href='#' onclick='return Meeted(\"" + full.biobookingid + "\")'><span class='glyphicon glyphicon-check'></span> เข้าพบแล้ว</a>"
                                                    + "&nbsp; <a class='btn btn-danger btn-sm' href='#' onclick='return UnMeet(\"" + full.biobookingid + "\")'><span class='glyphicon glyphicon-remove'></span> ไม่เข้าพบ</a>";
                                        } else if (String(full.biobookingstatus) === '1') {
                                            return '';
                                        } else if (String(full.biobookingstatus) === '-1') {
                                            return '';
                    ...
                    function CancelByID() {
                        if (!$.trim($("#rejectreason").val())) {
                            sudoNotify.error("กรุณากรอกเหตุผลการยกเลิก");
                            return false;
                        }
                        $.ajax({
                            url: '/cancelBioBookingByBookingid/' + $("#hidbiobookid").val() + '/' + $("#rejectreason").val(),
                            method: 'get',
                            dataType: 'json',
                            contentType: false,
                            processData: false,
                            ...
                            success: function (data) {
                                if (data.message === 'saved') {
                                    ...
                                    $('#mdlCancel').modal('toggle');
                                    sudoNotify.success("ยกเลิกการนัดพบ");
                                }
                            }
                        });
                    }
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.message === 'saved') {
                                    $('#tblReviewform').DataTable().ajax.reload();
                                    $('#mdlApprove').modal('toggle');
                                    sudoNotify.success("อนุมัติการนัดพบเรียบร้อย");
                                }
                            }
                        });
                    }
                    ...
                    function Meeted(id) {
                        $.ajax({
                            ...
                            success: function (data) {
                                if (data.message === 'saved') {
                                    $('#tblReviewform').DataTable().ajax.reload();
                                    sudoNotify.success("บันทึกข้อมูลการนัดพบเรียบร้อย");
                                }
                            }
                        });
                    }
