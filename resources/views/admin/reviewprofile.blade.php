...
<div class="page-content">
                    ...
                    <!-- END PAGE BAR -->
                    <!-- BEGIN PAGE TITLE-->
                    <h3 class="page-title"> {{$reviewuser->firstname}} Profile 
        ...
        <div class="alert alert-danger display-hide">
            <button class="close" data-close="alert"></button>
                 ...
                 <form role="form" class="profile-form form-bordered" action="{{ url('/profile') }}" enctype="multipart/form-data">
                        <input type="hidden" id="_userid" value="{{$reviewuser->id}}"/>
                        <div class="form-group">
                            <label class="control-label">คำนำหน้า</label>
                                <label class="control-label visible-ie8 visible-ie9">คำนำหน้าชื่อ</label>
                                
                                <input class="form-control placeholder-no-fix" type="text"  name="title" value="{{ $title }}" />
                            </div>
                        ...
                        <div class="form-group">
                            <label class="control-label">ชื่อจริง</label>
                                ...
                                <input class="form-control placeholder-no-fix" type="text" name="firstname" value="{{ old('firstname')!=null ? old('firstname') : $reviewuser->firstname}}" />
                        </div>
                        <div class="form-group">
                            ...
                            <label class="control-label">นามสกุล</label>
                                ...
                                <input class="form-control placeholder-no-fix" type="text" name="lastname" value="{{ old('lastname')!=null ? old('lastname') : $reviewuser->lastname}}" />
                            
                        </div>
                        <div class="form-group">
                            <label class="control-label">เพศ</label>
                                <label class="control-label visible-ie8 visible-ie9">เพศ</label>
                                <input class="form-control placeholder-no-fix" type="text"  name="sex" value="{{ $sex }}" />
                        </div>
                        <div class="form-group">
                            <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                            <label class="control-label">Email</label>
                                <label class="control-label visible-ie8 visible-ie9">Email</label>
                                <input class="form-control placeholder-no-fix" type="text" name="email" value="{{ old('email')!=null ? old('email') : $reviewuser->email}}" disabled/>
                        </div>
                        <div class="form-group">
                            <label class="control-label">เกิดวันที่</label>
                                <input data-date-format="dd-mm-yyyy" class="form-control form-control-inline date-picker" size="16" type="text" value="{{ old('dob')!=null ? old('dob') : $reviewuser->get_userDOB()}}" name="dob">
                        </div>
                        <div class="form-group">
                            <label class="control-label">สัญชาติ</label>
                                <label class="control-label visible-ie8 visible-ie9">สัญชาติ</label>
                                
                                <input class="form-control placeholder-no-fix" type="text"  name="nationality" value="{{ $nationality }}" />
                        </div>
                        <div class="form-group">
                            <label class="control-label">กลุ่มสมาชิก</label>
                                <label class="control-label visible-ie8 visible-ie9">กลุ่มสมาชิก</label>
                                <input class="form-control placeholder-no-fix" type="text"  name="type" value="{{ $type }}" />
                        </div>
                        <div class="form-group user_occupation_panel" >
                            <label class="control-label">ประเภทของบุคคลากร</label>
                                ...
                                <input class="form-control placeholder-no-fix" type="text"  name="occupation" value="{{ $occupation }}" />
                        </div>
                        <div id="pre-hidden-group">
                            <div>
                                <div class="form-group" hide student>
                                    <label class="control-label">รหัสนิสิต</label>
                                        ...
                                        <input class="form-control placeholder-no-fix" type="text"  name="student_id" value="{{ old('student_id')!=null ? old('student_id') : $reviewuser->student_id}}" />
                                </div>
                                <div class="form-group" hide student>
                                    <label class="control-label">อาจารย์ที่ปรึกษา</label>
                                        <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                                        <label class="control-label visible-ie8 visible-ie9">อาจารย์ที่ปรึกษา</label>
                                        <input class="form-control placeholder-no-fix" type="text"  name="advisor" value="{{ old('advisor')!=null ? old('advisor') : $reviewuser->advisor}}" />
                                </div>
                                <div class="form-group" hide student>
                                    <label class="control-label">หัวข้องานวิจัย</label>
                                        <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                                        <label class="control-label visible-ie8 visible-ie9">หัวข้องานวิจัย</label>
                                        <input class="form-control placeholder-no-fix" type="text"  name="researchtopic" value="{{ old('researchtopic')!=null ? old('researchtopic') : $reviewuser->researchtopic}}" /> 
                                </div>
                                <div class="form-group department-panel">
                                    <label class="control-label">ภาควิชา</label>
                                        ...
                                        <input class="form-control placeholder-no-fix" type="text"  name="department" value="{{ $department }}" />
                                </div>
                            </div>
                            <div class="form-group" hide outsider>
                                <label class="control-label">มหาวิทยาลัย/บริษัท</label>
                                    ...
                                    <input class="form-control placeholder-no-fix" type="text"  name="company" value="{{ old('company')!=null ? old('company') : $reviewuser->company}}" />
                            </div>
                            <div class="form-group">
                                <label class="control-label">เลขบัตรประจำตัวประชาชน</label>
                                    ...
                                    <input class="form-control placeholder-no-fix" type="text" name="citizen_id" value="{{ old('citizen_id')!=null ? old('citizen_id') : $reviewuser->citizen_id}}" />
                            </div>
                            <div class="form-group">
                                <label class="control-label">Passport ID</label>
                                    <!--ie8, ie9 does not support html5 placeholder, so we just show field title for that-->
                                    <label class="control-label visible-ie8 visible-ie9">Passport ID</label>
                                    <input class="form-control placeholder-no-fix" type="text" name="passport_id" value="{{ old('passport_id')!=null ? old('passport_id') : $reviewuser->passport_id}}" disabled="true" />
                            </div>
                            <div class="form-group">
                                <label class="control-label">ที่อยู่</label>
                                    <label class="control-label visible-ie8 visible-ie9">ที่อยู่</label>
                                    <input class="form-control placeholder-no-fix" type="text" name="address" value="{{ old('address')!=null ? old('address') : $reviewuser->address}}" />
                            </div>

                            <div class="form-group">
                                <label class="control-label">โทรศัพท์</label>
                                    <label class="control-label visible-ie8 visible-ie9">โทรศัพท์</label>
                                    <input class="form-control placeholder-no-fix" type="text" name="tel" value="{{ old('tel')!=null ? old('tel') : $reviewuser->tel}}" />
                            </div>
                            <div class="form-group">
...
@section('footer')
	<!-- BEGIN PAGE LEVEL PLUGINS -->
        ...
        <script src="/assets/pages/scripts/reviewprofile.js" type="text/javascript"></script>
